...
if os.path.exists(selectedImages):
    shutil.rmtree(selectedImages)
os.makedirs(selectedImages)

print("................Inside final evaluation.............")
...
for dir in dirList:
    ...
    with tf.device('/CPU:0'):
        ...
        if rank_images:
            ...
            if not os.path.exists(selectedImages):
                    os.makedirs(selectedImages)
            print("CWD:",os.getcwd())
            for i, (name, score) in enumerate(score_list):
                print("%d)" % (i + 1), "%s : Score = %0.5f" % (name, score))
                
                name = "highest_score_cand_images"+"/"+name
                # name = ""+str(shotNum)+"/"+name
                x = cv2.imread(name)
                count+=1
                cv2.imwrite(os.path.join(os.getcwd(),selectedImages,str(f"{count:05}")+".jpg"),x)
